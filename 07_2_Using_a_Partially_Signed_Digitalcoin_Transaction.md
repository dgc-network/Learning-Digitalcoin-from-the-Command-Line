# 7.2: Using a Partially Signed Digitalcoin Transaction

> :information_source: **NOTE:** This section has been recently added to the course and is an early draft that may still be awaiting review. Caveat reader.

Now that you've learned the basic workflow of generating a PSBT, you probably want to do something with it. What can PSBTs do that multi-sigs (and normal raw transactions) can't? To start with, you've got the ease of use of a standardized format, which means that you can use your `digitalcoin-cli` transactions and meld them with transactions generated by people (or programs) on other platforms. Beyond that, you can do some things that just weren't easy using other mechanics.

Following are three examples of using PSBTs for: multi-sigs, pooling money, and joining coins.

> :warning: **VERSION WARNING:** This is an innovation from Digitalcoin Core v 0.17.0. Earlier versions of Digitalcoin Core will not be able to work with the PSBT while it is in process (though they will still be able to recognize the final transaction).
...
```
machine1$ digitalcoin-cli -named addmultisigaddress nrequired=2 keys='''["'$pubkey1'","'$pubkey2'"]'''
{
...
machine1$ digitalcoin-cli -named importaddress address="tb1qyfxt2qa877p40xdecghwps78my7sjq6kuv88qq2u86al5526xp6qfqjud0" rescan=false

machine2$ digitalcoin-cli -named addmultisigaddress nrequired=2 keys='''["'$pubkey1'","'$pubkey2'"]'''
{
  "address": "tb1qyfxt2qa877p40xdecghwps78my7sjq6kuv88qq2u86al5526xp6qfqjud0",
  "redeemScript": "5221038d73adf2c7ea33f9dc34b77b62b59af433c1de9c763332da79e83e155f96030e2103789f543423670e169667ff7e1f2da2a97df1b0912272e142d582451acebd063652ae",
  "descriptor": "wsh(multi(2,[d6043800/0'/0'/26']038d73adf2c7ea33f9dc34b77b62b59af433c1de9c763332da79e83e155f96030e,[be686772]03789f543423670e169667ff7e1f2da2a97df1b0912272e142d582451acebd0636))#07zyayfk"
}
machine2$ digitalcoin-cli -named importaddress address="tb1qyfxt2qa877p40xdecghwps78my7sjq6kuv88qq2u86al5526xp6qfqjud0" rescan=false
```
...
machine1$ psbt=$(digitalcoin-cli -named createpsbt inputs='''[ { "txid": "'$utxo_txid'", "vout": '$utxo_vout' } ]''' outputs='''{ "'$split1'": 0.009998,"'$split2'": 0.009998 }''')
```
...
```
machine1$ psbt_p1=$(digitalcoin-cli walletprocesspsbt $psbt | jq -r '.psbt')
machine1$ digitalcoin-cli decodepsbt $psbt_p1
{
  "tx": {
...
machine1$ digitalcoin-cli analyzepsbt $psbt_p1
{
  "inputs": [
    {
      "has_utxo": true,
...
machine2$ psbt_p2=$(digitalcoin-cli walletprocesspsbt $psbt | jq -r '.psbt')
...
machine2$ psbt_c=$(digitalcoin-cli combinepsbt '''["'$psbt_p1'", "'$psbt_p2'"]''')
$ digitalcoin-cli decodepsbt $psbt_c
{
  "tx": {
    "txid": "ee82d3e0d225e0fb919130d68c5052b6e3c362c866acc54d89af975330bb4d16",
    "hash": "ee82d3e0d225e0fb919130d68c5052b6e3c362c866acc54d89af975330bb4d16",
...
$ digitalcoin-cli analyzepsbt $psbt_c
{
  "inputs": [
    {
      "has_utxo": true,
...
```
machine2$ psbt_c_hex=$(digitalcoin-cli finalizepsbt $psbt_c | jq -r '.hex')
standup@btctest2:~$ digitalcoin-cli -named sendrawtransaction hexstring=$psbt_c_hex
ee82d3e0d225e0fb919130d68c5052b6e3c362c866acc54d89af975330bb4d16
```
Obviously, there wasn't a big improvement in using this method over serially signing a transaction for a 2-of-2 multisig when everyone was using `digitalcoin-cli`: we could have passed a raw transaction with partial signatures from one user to the other just as easily as sending that PSBT. But, this was the simplest case. As we delve into more complex multisigs, this methodology becomes better and better. 

First of all, it's platform independent. As long as everyone is using a service that supports Digitalcoin Core 0.17, they'll all be able to sign this transaction, which isn't true when classic multi-sigs are being passed around among different platforms. 
...
The traditional answer is to create a multisig, then to have the participants individually send their funds to it. The problem is that the first payer has to depend on the good faith of the second, and that doesn't built on the strength of Digitalcoin, which is its _trustlessness_. Fortunately, with the advent of PSBTs, we can now make trustless payments that pool funds.
...
machine2$ digitalcoin-cli listunspent
[
 {
...
And create a PSBT:
```
machine1$ psbt=$(digitalcoin-cli -named createpsbt inputs='''[ { "txid": "'$utxo_txid_1'", "vout": '$utxo_vout_1' }, { "txid": "'$utxo_txid_2'", "vout": '$utxo_vout_2' } ]''' outputs='''{ "'$multisig'": 0.019998 }''')
```
Here's what it looks like:
```
machine1$ digitalcoin-cli decodepsbt $psbt
{
  "tx": {
    "txid": "53ec62c5c2fe8b16ee2164e9699d16c7b8ac30ec53a696e55f09b79704b539b5",
...
```
machine1$ digitalcoin-cli walletprocesspsbt $psbt
{
...
machine2$ psbt_f=$(digitalcoin-cli walletprocesspsbt $psbt_p | jq -r '.psbt')
machine2$ digitalcoin-cli analyzepsbt $psbt_f
{
  "inputs": [
    {
      "has_utxo": true,
...
machine2$ psbt_hex=$(digitalcoin-cli finalizepsbt $psbt_f | jq -r '.hex')
machine2$ digitalcoin-cli -named sendrawtransaction hexstring=$psbt_hex
...
CoinJoin is another Digitalcoin application that requires trustlessness. Here, you have a variety of parties who don't necessarily know each other joining money and getting it back.
...
```
$ psbt=$(digitalcoin-cli -named createpsbt inputs='''[ { "txid": "'$utxo_txid_1'", "vout": '$utxo_vout_1' }, { "txid": "'$utxo_txid_2'", "vout": '$utxo_vout_2' }, { "txid": "'$utxo_txid_3'", "vout": '$utxo_vout_3' } ]''' outputs='''{ "'$split1'": 1.7,"'$split2'": 0.93,"'$split3'": 1.4 }''')
```
...
Continue "Expanding Digitalcoin Transactions with PSBTs" with [§7.3: Inegrating with Hardware Wallets](07_3_Integrating_with_Hardware_Wallets.md).
