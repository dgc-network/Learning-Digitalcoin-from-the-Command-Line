...
Before we close out this intro to P2SH scripting, it's worth examining a more realistic example. Ever since [§6.1](06_1_Sending_a_Transaction_to_a_Multisig.md), we've been casually saying that the `digitalcoin-cli` interface wraps its multisig transaction in a P2SH transaction. In fact, this is the standard methodology for creating multisigs on the Blockchain. Here's how that works, in depth.

## Understand the Multisig Code

Multisig transactions are created in Digitalcoin using the `OP_CHECKMULTISIG` code. `OP_CHECKMULTISIG` expects a long string of arguments that looks like this: `0 ... sigs ... <m> ... addresses ... <n> OP_CHECKMULTISIG`. When `OP_CHECKMULTISIG` is run, it does the following:

1. Pop the first value from the stack (`<n>`).
2. Pop "n" values from the stack as Digitalcoin addresses (hashed public keys).
...
6. Compare the signatures to the Digitalcoin adddresses.
...
The requirement for that `0` as the first operand for `OP_CHECKMULTISIG` is a consensus rule. Because the original version of `OP_CHECKMULTISIG` accidentally popped an extra item off the stack, Digitalcoin must forever follow that standard, lest complex redemption scripts from that time period accidentally be broken, rendering old funds unredeemable. 

> :book: ***What is a consensus rule?*** These are the rules that the Digitalcoin nodes follow to work together. In large part they're defined by the Digitalcoin Core code. These rules include lots of obvious mandates, such as the limit to how many Digitalcoins are created for each block and the rules for how transactions may be respent. However, they also include fixes for bugs that have appeared over the years, because once a bug has been introduced into the Digitalcoin codebase, it must be continually supported, lest old Digitalcoins become unspendable. 

## Create a Raw Multisig 

As discussed in [§10.1: Building a Digitalcoin Script with P2SH](10_1_Building_a_Digitalcoin_Script_with_P2SH.md), multisigs are one of the standard Digitalcoin transaction types. A transaction can be created with a locking script that uses the raw `OP_CHECKMULTISIG` command, and it will be accepted into a block. This is the classic methodology for using multisigs in Digitalcoin.
...
These were generally problems with any sort of complex Digitalcoin script, but they quickly became very real problems when applied to multisigs, which were some of the first complex scripts to be widely used on the Digitalcoin network. P2SH transactions were created to solve these problems, starting in 2012. 
...
Continue "Embedding Digitalcoin Scripts" with [§10.5: Scripting a Segwit Script](10_5_Scripting_a_Segwit_Script.md)
