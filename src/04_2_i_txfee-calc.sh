...
usedtxid=($(digitalcoin-cli decoderawtransaction $1 | jq -r '.vin | .[] | .txid'))
usedvout=($(digitalcoin-cli decoderawtransaction $1 | jq -r '.vin | .[] | .vout'))
...
btcout=$(digitalcoin-cli decoderawtransaction $1 | jq -r '.vout  [] | .value' | awk '{s+=$1} END {print s}')
