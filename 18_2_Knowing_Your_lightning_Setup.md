...
> :link: **TESTNET vs MAINNET:** If you're using mainnet, then _everything_ will instead be placed in the main `~/.lightning/digitalcoin` directory. These various setups _do_ elegantly stack, so if you are using mainnet, testnet, and regtest, you'll find that `~/.lightning/digitalcoin` contains your config file and your mainnet data, the `~/.lightning/testnet` directory contains your testnet data, and the `~/.lightning/regtest` directory contains your regtest data.

## Know Your lightning-cli Commands

Most of your early work will be done with the `lightning-cli` command, which offers an easy interface to `lightningd`, just like `digitalcoin-cli` does.
...
withdraw destination satoshi [feerate] [minconf] [utxos]
    Send to {destination} address {satoshi} (or 'all') amount via Digitalcoin transaction, at optional {feerate}
...
estimatefees 
    Get the urgent, normal and slow Digitalcoin feerates as sat/kVB.
...
getrawblockbyheight height
    Get the digitalcoin block at a given height
...
sendrawtransaction tx
    Send a raw transaction to the Digitalcoin network.
...
dev-rescan-outputs 
    Synchronize the state of our funds with digitalcoind
...
c$ lightning-cli --testnet listconfigs
{
   ...
   "plugins": [
      ...
      },
      {
         ...
         "name": "bcli",
         "options": {
            "digitalcoin-datadir": null,
            "digitalcoin-cli": null,
            "digitalcoin-rpcuser": null,
            "digitalcoin-rpcpassword": null,
            "digitalcoin-rpcconnect": null,
            "digitalcoin-rpcport": null,
            "digitalcoin-retry-timeout": null,
            "commit-fee": "500"
         }
      },
      {
