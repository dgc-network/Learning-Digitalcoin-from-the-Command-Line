...
The methodology described in this chapter for integrating with a hardware wallet depends on the [Digitalcoin Hardware Wallet Interface](https://github.com/digitalcoin-core/HWI) released through Digitalcoin Core and builds on the [installation](https://github.com/digitalcoin-core/HWI/blob/master/README.md) and [usage](https://github.com/digitalcoin-core/HWI/blob/master/docs/digitalcoin-core-usage.md) instructions found there.

> :warning **FRESHNESS WARNING:** The HWI interface is very new and raw around the edges as of Digitalcoin Core v 0.20.0. It may be hard to install correctly, and it may have unintuive errors. What follows is a description of a working setup, but it took several tries to get it right, and your setup may vary.

## Install Digitalcoin Core on a Local Machine

_If you just plan to read over this section and not test out these commands until some future date when you have a local development environment, you can skip this subsection, which is about creating a Digitalcoin Core installation on a local machine such as a Mac or UNIX machine._

There are alternate versions of the Digitalcoin Standup script that you used to create your VM that will install on a MacOS or on a non-Linode UNIX machine.

If you have MacOS, you can install [Digitalcoin Standup MacOS](https://github.com/BlockchainCommons/Digitalcoin-Standup-MacOS/blob/master/README.md).

If you have a local UNIX machine, you can install [Digitalcoin Standup Linux Scripts](https://github.com/BlockchainCommons/Digitalcoin-Standup-MacOS/blob/master/README.md).

Once you've gotten Digitalcoin Standup running on your local machine, you'll want to sync the "Testnet" blockchain, assuming that you're continuing to follow the standard methodlogy of this course.

We will be using a Macintosh and Testnet for the examples in this section.

### Create an Alias for Digitalcoin-CLI

Create an alias that runs `digitalcoin-cli` from the correct directory with any appropriate flags.
...
HWI is a Digitalcoin Core program available in python that can be used to interact with hardware wallets.
...
```
$ cd ~/StandUp
$ git clone https://github.com/digitalcoin-core/HWI.git
```
...
2. Go to the "Manager" and install "Digitalcoin Test". The current version requires that you have "Digitalcoin" and "Etereum" installed first.
3. Go to the "Manager", scroll to your new "Digitalcoin Test", and "Add Account"

## Link to a Ledger

In order for a Ledger to be accessible, you must login with your PIN and then call up the app that you want to use, in this case the "Digitalcoin Test" app. You may need to repeat this from time to time if your Ledger falls asleep.
...
You can watch for funds by importing addresses from your hardware wallet to your full node, using HWI and `digitalcoin-cli`.

### Create a Wallet

To use your hardware wallet with `digitalcoin-cli`, you'll want to create a specific named wallet in Digitalcoin Core, using the `createwallet` RPC, which is a command we haven't previously discussed.
```
$ digitalcoin-cli --named createwallet wallet_name="ledger" disable_private_keys="true"
{
  "name": "ledger",
  "warning": ""
}
...
The values returned by `getkeypool` are the same sorts of descriptors that we learned about in [§3.5: Understanding the Descriptor](03_5_Understanding_the_Descriptor.md). At the time, we said that they were most useful for moving addresses among different machines. Here's the real-life example: moving addresses from a hardware wallet to Digitalcoin Core node, so that our net-connected machine will be able to watch over the keys owned by the offline hardware wallet.
...
```
$ digitalcoin-cli getdescriptorinfo "wpkh([9a1d520b/84h/1h/0h]tpubDD7KTtoGzK9GuWUQcr1uTJazsAkqoXhdrwGXWVix6nPpNZmSbagZWD4QSaMsyK8YohAirGDPrWdRiEpKzTFB7DrTrqfzHCn7yi5EsqeR93S/0/*)#qttxy592"
{
...
```
$ digitalcoin-cli -rpcwallet=ledger importmulti '[{"desc": "wpkh([9a1d520b/84h/1h/0h]tpubDD7KTtoGzK9GuWUQcr1uTJazsAkqoXhdrwGXWVix6nPpNZmSbagZWD4QSaMsyK8YohAirGDPrWdRiEpKzTFB7DrTrqfzHCn7yi5EsqeR93S/0/*)#qttxy592", "range": [0, 1000], "timestamp": "now", "internal": false, "keypool": true, "active": true, "watchonly": true}, {"desc": "wpkh([9a1d520b/84h/1h/0h]tpubDD7KTtoGzK9GuWUQcr1uTJazsAkqoXhdrwGXWVix6nPpNZmSbagZWD4QSaMsyK8YohAirGDPrWdRiEpKzTFB7DrTrqfzHCn7yi5EsqeR93S/1/*)#3lw8ep4j", "range": [0, 1000], "timestamp": "now", "internal": true, "keypool": true, "active": true, "watchonly": true}]'
[
  {
    "success": true
  },
...
```
$ digitalcoin-cli -rpcwallet=ledger getaddressesbylabel "" | more
{
...
```
$ digitalcoin-cli -rpcwallet=ledger getnewaddress
tb1qqqvnezljtmc9d7x52udpc0m9zgl9leugd2ur7y
```
Then you send money to it.

The power of HWI is that you can watch the payments from your Digitalcoin Core node, rather than having to plug in your hardware wallet and query it.
```
$ digitalcoin-cli -rpcwallet=ledger listunspent
[
  {
...
```
$ digitalcoin-cli -named -rpcwallet=ledger walletcreatefundedpsbt inputs='''[]''' outputs='''[{"tb1qcaedd724gts3aug73m78c7nfsv9d8zs9q6h2kd":0.015}]'''
{
...
$ digitalcoin-cli decodepsbt $psbt
{
  "tx": {
...
```
$ digitalcoin-cli analyzepsbt $psbt
{
  "inputs": [
    {
      "has_utxo": true,
...
```
Because you imported that keypool, `digitalcoin-cli` has all the information it needs to fill in the inputs, it just can't sign because the private keys are held on the hardware wallet.
...
```
$ digitalcoin-cli analyzepsbt $psbt_f
{
  "inputs": [
    {
      "has_utxo": true,
...
```
$ digitalcoin-cli finalizepsbt $psbt_f
{
...
$ hex=
$ digitalcoin-cli sendrawtransaction $hex
...
Hardware wallets can offer better protection by keeping your private keys offline, protected in the hardware. Fortunately, there's still a way to interact with them using `digitalcoin-cli`. You just install HWI and it will then allow you to (1) import public keys to watch them; and (2) sign transactions using your hardware wallet.

> :fire: ***What's the power of HWI?*** HWI lets you interact with hardware wallets using all of the commands of `digitalcoin-cli` that you've learned to date. You can make raw transactions of any sort, then send PSBTs to  hardware wallets for signing. Thus, you have all of the power of Digitalcoin Core, but you also have the security of a hardware device.
