...
This section explains how to interact with `digitalcoind` using the Swift programming language and your own RPC client.
...
Finally, you'll need your `rpcuser` and `rpcpassword` information. That's in `~/Library/Application Support/Digitalcoin/digitalcoin.conf` by default under Gordian.
```
$ grep rpc ~/Library/Application\ Support/Digitalcoin/digitalcoin.conf
rpcuser=oIjA53JC2u
...
At the time of this writing, there isn't an up-to-date, simple-to-use Digitalcoin RPC Library that's specific for Swift, something that you can drop in and immediately start using. Thus, you're're going to do something you're never done before: build an RPC connection by hand.

### Write the RPC Transmitter

This just requires writing a function that passes RPC commands on to `digitalcoind` in the correct format:
```
func makeCommand(method: String, param: Any, completionHandler: @escaping (Any?) -> Void) -> Void {
```

RPC connections to `digitalcoind` use the HTML protocol, which means that you need to do three things: create a URL; make a URLRequest; and initiate a URLSession.
...
```
    ...
    let walletName = ""
```
The actual RPC connection to Digitalcoin Core is built using a URL of the format "http://rpcusername:rpcpassword@nodeIp/walletName":
```
...
With that URL in you hand, you can now create a URLRequest, with the `POST` method and the `text/plain` content type. The HTTP body is then the familiar JSON object that you've been sending whenever you connect directly to Digitalcoin Core's RPC ports, as first demonstrated when using Curl in [§4.4](04_4__Interlude_Using_Curl.md).
```
...
> :pray: **THANKS:** Thanks to @Fonta1n3 who provided the [main code](https://github.com/BlockchainCommons/Learning-Digitalcoin-from-the-Command-Line/issues/137) for our RPC Transmitter.
...
param = "\"learning-digitalcoin\", \"legacy\""
...
```
$ digitalcoin-cli getaddressesbylabel "learning-digitalcoin"
{
  "mt3ZRsmXHVMMqYQPJ8M74QjF78bmqrdHZF": {
    "purpose": "receive"
  }
...
That covers our usual discussions of programming Digitalcoin RPC in a language, but Swift is a particularly important language since it can be deployed on mobile devices, one of the prime venues for wallets. As such, you may wish to consider a few other libraries:
