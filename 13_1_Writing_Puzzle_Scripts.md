...
Digitalcoin Scripts _don't_ actually have to depend on the knowledge of a secret key. They can instead be puzzles of any sort. 
...
More notably, creating alegebraic puzzles gives you a nice understanding of how the arithmetic functions in Digitalcoin Script work.

### Write a Multiplier Script

Digitalcoin Script has a number of opcodes that were disabled to maintain the security of the system. One of the is `OP_MUL`, which would have allowed multiplication ... but is instead disabled.
...
> :warning: **WARNING** `btcdeb` isn't just useful for providing visualization of these scripts, but to also double-check the results. Sure enough, we got this one wrong the first time, testing the equations in the wrong order. That's how easy it is to make a financially fatal mistake in a Digitalcoin Script, and that's why every script must be tested.
...
Puzzle scripts are great to further examine Digitalcoin Scripting, but you'll only see them in real-world use if they're holding small amounts of funds or if they're intended for redemption by very skilled users. There's a reason for this: they aren't secure. 
...
Puzzles scripts are a great introduction to more realistic and complex Digitalcoin Scripts. They demonstrate the power of the mathematical and stack functions in Digitalcoin Script and how they can be carefully combined to create questions that require very specific answers. However, their real-world usage is also limited by the security issues inherent in non-signed Digitalcoin transactions.
...
Continue "Designing Real Digitalcoin Scripts" with [§13.2: Writing Complex Multisig Scripts](13_2_Writing_Complex_Multisig_Scripts.md).
