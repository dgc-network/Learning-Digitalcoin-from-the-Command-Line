...
```
$ digitalcoin-cli -named getnewaddress address_type=p2sh-segwit
...
```
$ digitalcoin-cli getnewaddress
tb1q5gnwrh7ss5mmqt0qfan85jdagmumnatcscwpk6
```
As we've already seen, change addresses generated from within `digitalcoin-cli` interact fine with Bech32 addresses, so there's no point in using the `legacy` flag there either:
```
$ digitalcoin-cli getrawchangeaddress
...
Digitalcoin-cli doesn't care which address type you're using. You can run a command like `listaddressgroupings` and it will freely mix addresses of the different types:
```
$ digitalcoin-cli listaddressgroupings
[
  [
    [
...
So how do you send a Digitalcoin transaction? Exactly like any other transaction. It doesn't matter if the UTXO is SegWit, the address is SegWit, or some combination thereof. You can expect `digitalcoin-cli` to do the right thing. Though you can tell the differences via the addresses, they don't matter for interacting with things at the `digitalcoin-cli` or RPC level. (And this is one of the advantages of using the command line and the RPC interface, as suggested in this tutorial: experts have already done the hard work for you, including things like how to send to both legacy and Bech32 addresses. You just get to use that functionality to your own advantage.)

Here's an example of sending to a SegWit address, the easy way:
```
$ digitalcoin-cli sendtoaddress tb1qw508d6qejxtdg4y5r3zarvary0c5xw7kxpjzsx 0.005
...
```
$ digitalcoin-cli gettransaction txid="854a833b667049ac811b4cf1cad40fa7f8dce8b0f4c1018a58b84559b6e05f42" verbose=true
{
  "amount": -0.00500000,
  "fee": -0.00036600,
  "confirmations": 0,
...
```
$ digitalcoin-cli -named gettransaction txid="33173618421804343e8f6cc21316d97a24f7439137510249af39f01ee82113a7"
{
  "amount": 0.01000000,
  "confirmations": 43,
...
```
$ changeaddress=$(digitalcoin-cli getrawchangeaddress)
$ echo $changeaddress
...
$ utxo_txid=$(digitalcoin-cli listunspent | jq -r '.[2] | .txid')
$ utxo_vout=$(digitalcoin-cli listunspent | jq -r '.[2] | .vout')
$ echo $utxo_txid $utxo_vout
003bfdca5578c0045a76768281f05d5e6f57774be399a76f387e2a0e99e4e452 0
$ rawtxhex=$(digitalcoin-cli -named createrawtransaction inputs='''[ { "txid": "'$utxo_txid'", "vout": '$utxo_vout' } ]''' outputs='''{ "'$recipient'": 0.002, "'$changeaddress'": 0.007 }''')
