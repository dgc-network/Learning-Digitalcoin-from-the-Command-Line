...
Converting a PSBT into Libwally's internal structure is incredibly easy, you just run `wally_psbt_from_base64` with a base64 PSBT — which are the outputs produced by `digitalcoin-cli`, such as:
...
```
And of course, you can check this with the `decodepsbt` RPC command for `digitalcoin-cli`:
```
...
```
You can even use `digitalcoin-cli` to test the result:
```
$ psbt=$(./createpsbt)
$ digitalcoin-cli decodepsbt $psbt
{
...
Learn more about "Programming Digitalcoin with Libwally" in [16.5: Using Scripts in Libwally](16_5_Using_Scripts_in_Libwally.md).
