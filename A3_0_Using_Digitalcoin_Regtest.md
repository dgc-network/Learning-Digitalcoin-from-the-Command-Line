# Appendix III: Using Digitalcoin Regtest 
...
On regtest, usually there are not enough transactions so digitalcoind cannot give a reliable estimate and, without it, the wallet will not create transactions unless it is explicitly set the fee.

### Reset the Regtest Blockchain

If you wish, you can later restart your Regtest with a new blockchain. 

Regtest wallets and blockchain state (chainstate) are saved in the regtest subdirectory of the Digitalcoin configuration directory:
```
user@mybtc:~/.digitalcoin# ls
...
Before generating blocks, it is necessary to load a wallet using `loadwallet` or create a new one with `createwallet`. Since version 0.21, Digitalcoin Core does not automatically create new wallets on startup.
...
```
$ digitalcoin-cli -regtest -named createwallet wallet_name="regtest_desc_wallet" descriptors=true
```
...
```
$ digitalcoin-cli -regtest -generate 101
[
...
```
$ digitalcoin-cli -regtest getbalance
50.00000000
```
...
```
$ digitalcoin-cli -regtest sendtoaddress [address] 15.1
...
```
$ digitalcoin-cli -regtest gettransaction e834a4ac6ef754164c8e3f0be4f34531b74b768199ffb244ab9f6cb1bbc7465a
{
  "amount": 0.00000000,
  "fee": -0.00178800,
  "confirmations": 0,
...
```
$ digitalcoin-cli -regtest -generate 6
[
...
## Summary: Using Digitalcoin Regtest

A regtest environment for Digitalcoin works just like any testnet environment, except for the fact that you have the ability to easily and quickly generate blocks.
...
But otherwise, you've reached the end! Other people who have worked their way through this course have become professional Digitalcoin developers and engineers, including some of whom have contributed to [Blockchain Commons](https://www.blockchaincommons.com/). We encourage you to do the same! Just get out there are start working on some of your own Digitalcoin code with what you learned.
